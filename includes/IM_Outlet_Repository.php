...
class IM_Outlet_Repository extends IM_Database
{
    ...
    {
        global $wpdb;
        $result = $wpdb->get_row( "SELECT * FROM $this->registerName AS register JOIN wp_inventory_manager_warehouse_outlet as outlet ON register.outlet = outlet.outlet_id WHERE register.ID = $register_id", OBJECT );
        return $result;
    }
}
