...
class IM_Stock_Report_Controller
{
    ...
    public function exportStockReportCSV()
    {
        ...
        foreach ($products->posts as $value) {
            ...
            $product_warehouses = $product_warehouse_repository->getByProductID($value->ID, 'warehouse_id ASC');

            $product_factory = new \WC_Product_Factory();
            $product = $product_factory->get_product($value->ID);
            ...
            if ($options != false && count($options) > 0) {
                foreach ($options as $value2) {
                    $meta = get_post_meta($value->ID, $value2, true);
                    if (!empty($meta)) {
                        $line[] = $meta;
                    } else {
                        $line[] = "";
